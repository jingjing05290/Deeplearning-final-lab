...
print(data.shape)
if not os.path.exists('VAE_masked_1'):  #selected_points = masked_input_data[1].cpu().numpy()  # (3,96,96)
    os.makedirs('VAE_masked_1')
...
# Modify the bbox2mask function for the 3D data format
def bbox2mask_3d(bboxes, config):
    _, height, width = config['image_shape']
    max_delta_h, max_delta_w = config['max_delta_shape']
    outpaint = config['outpaint']
    ...
    for i in range(batch_size):
        ...
        y1, x1, y2, x2 = bbox[0].item(), bbox[1].item(), bbox[2].item(), bbox[3].item()
        mask[i, y1 + delta_h:y2 - delta_h, x1 + delta_w:x2 - delta_w] = 0. if outpaint else 1.

    return mask



# Function to apply mask to the dataset
def apply_mask_to_dataset(dataset, config, device):
    masked_dataset = []
    for data in dataset:
        bboxes = torch.tensor([[42, 42, 54, 54]], device=device)  # 在指定设备上创建边界框
        mask = bbox2mask_3d(bboxes, config).to(device)  # 确保遮罩在同一设备上
        masked_data = data * (1 - mask)
        masked_dataset.append(masked_data)
...
bboxes = torch.tensor([[42, 42, 54, 54]])
...
for epoch in range(epochs):
    ...
    for batch_idx, (data,) in enumerate(train_loader):
        ...
        train_masked_list = apply_mask_to_dataset(data, config, device)
        train_masked = torch.stack(train_masked_list)


        mu, logvar = encoder(train_masked)
        z = encoder.reparameterize(mu, logvar)
        recon_data = decoder(z)

        recon_data_2d = recon_data[:, :2, :, :].detach().cpu().numpy()
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            test_masked_list = apply_mask_to_dataset(input_data, config, device)
            test_masked = torch.stack(test_masked_list)
            mu, logvar = encoder(test_masked)
            z = encoder.reparameterize(mu, logvar)
            recon_data = decoder(z)

            y1, x1, y2, x2 = 50, 50, 70, 70
            masked_recon_data = recon_data[:, :, y1:y2, x1:x2]
            masked_input_data = input_data[:, :, y1:y2, x1:x2]

            error = calculate_relative_error(masked_recon_data, masked_input_data)
            ...
            if epoch%30 == 0:
                decoded_masked_points = masked_recon_data[1].cpu().numpy() #(3,20,20)
                plot_magfield(decoded_masked_points, vmax=0.25)
                plt.title('decoder masked Data Points')
                plt.savefig(f'VAE_masked_1/decoder masked data {epoch}.png')

                decoded_selected_points = recon_data[1].cpu().numpy()  # (3,96,96)
                plot_magfield(decoded_selected_points, vmax=0.25)
                plt.title('Original decoder Data Points')
                plt.savefig(f'VAE_masked_1/original decoder data{epoch}.png')
                plt.close('all')



            if epoch == 1:
                selected_masked_points = masked_input_data[1].cpu().numpy()  # (3,20,20)
                plot_magfield(selected_masked_points, vmax=0.25)
                plt.title('Original masked Data Points')
                plt.savefig(f'VAE_masked_1/original masked data.png')

                selected_points = input_data[1].cpu().numpy()  # (3,96,96)
                plot_magfield(selected_points, vmax=0.25)
                plt.title('Original Data Points')
                plt.savefig(f'VAE_masked_1/original  data.png')
                plt.close('all')
